...
// Story 7.1: same-origin proxy for the Question Set generation command
// (mirrors .../shortlist/route.ts's exact shape) — the gateway is a
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ candidateId: string }> }) {
  ...
  try {
    gatewayResponse = await gatewayFetch(
      `/workspace/candidates/${encodeURIComponent(candidateId)}/question-set${revisionQuery}`,
      {
        method: "POST",
