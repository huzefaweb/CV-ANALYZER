...
- **Graphify** — a local knowledge graph covering both this codebase and the `_bmad-output/planning-artifacts/**` specs (epics, architecture, PRD, UX) lives in `graphify-out/`. Code extraction is tree-sitter/AST, free, no LLM. Doc/spec extraction is semantic, done via the `/graphify` skill's own parallel subagents reading the files directly — never the raw `graphify extract --backend <llm>` CLI, which has a known manifest bug for gitignored paths (deleting `manifest.json` to unstick it silently drops `_bmad-output/` from discovery instead of fixing it — if that happens, recover via `/graphify . --update`, not by touching the CLI further). Prefer `graphify query "<question>"` / `graphify explain <node>` / `graphify path A B` over cold grep-and-read sweeps for both "where is X in the code" and "what does Story N's AC actually say" questions. Rebuild with `/graphify . --update` after any code or spec-doc change in the area you're touching — its `graphify-out/` state is gitignored, so absence from `git status` is not evidence it's missing or current.
